...
* **歌詞校時工具**：按 `E` 進入校時模式，用空白鍵敲點每句開始時間，可匯出 `.lrc`；
  也可以直接把 `.lrc` 拖進畫面套用。
* **手機友善**：長句會自動折行放大（最多 6 行），字幕只會落在標題列與播放列
  之間的空間，不會被工具列蓋住；小尺寸手機（iPhone SE 這類 375px / 320px 寬）的
  播放列會收成兩排，3D 場景依視窗比例縮放。
...
|---|---|---|---|---|
| `空白` / `K` | 播放 / 暫停 | | `S` | 選歌 |
| `←` `→` | 倒退 / 快進 10 秒（`Shift` 30 秒）| | `L` | 側邊歌詞 |
| `↑` `↓` | 音量 | | `M` | 靜音 |
| `0`–`9` | 跳到 0%–90% | | `Home` | 回到開頭 |
| `N` `P` | 下一首 / 上一首（`P` 播超過 4 秒先回到開頭）| | `F` | 全螢幕 |
| `R` | 循環：全部 → 單曲 → 不循環 | | `Q` | 畫質（高 / 中 / 低）|
| `E` | 歌詞校時 | | `?` | 快捷鍵說明 |
| `[` `]` | 整首歌詞位移 ±0.1 秒 | | `Esc` | 關閉視窗 |

**手勢（滑鼠與觸控相同）**：點畫面播放 / 暫停（控制列隱藏時，第一下只叫出控制列）；
雙擊左 / 右側倒退 / 快進 10 秒；雙擊中間全螢幕。滑過波形會顯示該時間點與那句歌詞。

**會記住的設定**：上次聽的歌與位置（選歌畫面有「繼續播放」）、音量 / 靜音、循環模式、
畫質、側邊歌詞開關。畫面持續低於 28 FPS 時會自動降一級畫質（手動按過 `Q` 就不會再自動調）。
系統若開啟「減少動態效果」，鏡頭晃動與閃光會自動減弱。手機鎖定畫面 / 耳機按鍵可直接控制播放
（Media Session）。

播放列上另有 `↺ 10` / `10 ↻` 按鈕，以及時間輸入框 —— 直接輸入 `1:23`、`83`
或 `0:01:23` 按 Enter 就會跳到該時間（點一下時間顯示也會把目前時間填進去）。
